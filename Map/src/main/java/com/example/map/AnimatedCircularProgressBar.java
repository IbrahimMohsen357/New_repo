...
public class AnimatedCircularProgressBar extends Pane {
    ...
    private void updateLayout() {
        ...
        // استنى بعد الرسم لتحديث مكان النص بدقة
        Platform.runLater(() -> {
    ...
    private void setupAnimation() {
        ...
        timeline = new Timeline(
                new KeyFrame(Duration.millis(50), e -> {
                    ...
                    double target = targetProperty.get();
                    if (Math.abs(currentProgress - target) < 0.1) {
                        progressProperty.set(target);
                        timeline.stop();
                        return;
                    }
                    if (currentProgress < target) {
                        progressProperty.set(currentProgress + 1);
                    } else {
                        progressProperty.set(currentProgress - 1);
                    }
                })
    ...
    private void updateProgress() {
        ...
        percentText.setText((int) currentProgress + "%");
    ...
    public void restartAnimation() {
        if (timeline != null) {
            timeline.stop(); // إيقاف الأنيميشن الحالي
        }
        progressProperty.set(0); // إعادة التقدم للصفر
        setupAnimation(); // إعادة تشغيل الأنيميشن
    }
